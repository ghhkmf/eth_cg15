...
void Mesh::sampleSurface(ShapeQueryRecord & sRec,
...
Point3f Mesh::getInterpolatedVertex(uint32_t index, const Vector3f &bc) const {
	return (bc.x() * m_V.col(m_F(0, index)) + bc.y() * m_V.col(m_F(1, index))
			+ bc.z() * m_V.col(m_F(2, index)));
}

Normal3f Mesh::getInterpolatedNormal(uint32_t index, const Vector3f &bc) const {
	return (bc.x() * m_N.col(m_F(0, index)) + bc.y() * m_N.col(m_F(1, index))
...
void Mesh::setHitInformation(uint32_t index, const Ray3f &ray,
	...
	if (m_UV.size() > 0)
		its.uv = bary.x() * m_UV.col(idx0) + bary.y() * m_UV.col(idx1)
	...
	if (m_N.size() > 0) {
		...
		its.shFrame = Frame(
				(bary.x() * m_N.col(idx0) + bary.y() * m_N.col(idx1)
...
Point3f Mesh::getCentroid(uint32_t index) const {
	return (1.0f / 3.0f)
			* (m_V.col(m_F(0, index)) + m_V.col(m_F(1, index))
					+ m_V.col(m_F(2, index)));
}
